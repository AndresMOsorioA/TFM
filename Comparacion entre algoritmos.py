...
class GeneticAlgorithm:
    ...
    # Buscar la mejor generación según el mejor fitness de cada generación
        ...
        for g in range(len(self.history_occ)):
            ...
            occ = np.array(self.history_occ[g])
            ar = np.array(self.history_ar[g])

        # Fitness utilizado por el algoritmo
            fitness = ar * 60 + occ * 40
    ...
    # Datos únicamente de la mejor generación

        occ = np.array(self.history_occ[best_generation])
        ar = np.array(self.history_ar[best_generation])
    ...
    # Ordenar el frente de Pareto por occupancy

        front = sorted(front, key=lambda i: occ[i])

    # Gráfico

        fig, ax = plt.subplots(figsize=(11, 8))

    # Todos los individuos de la mejor generación
        ax.scatter(
            occ,
            ar,
            s=35,
    ...
    # Marcar el mejor individuo según fitness
    ...
    # Información
        ...
        ax.set_title(
            f"Frente de Pareto - Mejor generación "
            f"(Generación {best_generation + 1})"
        )
    ...
    # Mostrar información por consola
        print("\n" + "=" * 50)
        print("MEJOR GENERACIÓN")
        print("=" * 50)
        print(f"Generación: {best_generation + 1}")
        print(f"Mejor Fitness: {best_fitness:.3f}")
        print(f"Answer Rate: {ar[best_idx]:.2%}")
        print(f"Occupancy: {occ[best_idx]:.2%}")
        print(f"Individuos en el frente de Pareto: {len(front)}")
        print("=" * 50)
